module type SingleType = sig 
  type t
end

module type ObjReqAbsQueue = functor (Obj : SingleType) -> sig
  type t

  type slot = Obj of Obj.t
            | Query of (Obj.t -> unit)

  (* FIXME : choose better names *)
  type queue_state = Empty
                   | Normal 
                   | Request
...
module type ObjReqAbsHashtbl = sig
  module Queue : ObjReqAbsQueue

  val get_queue : 'a -> 'b
end

module ObjReqQueue : ObjReqAbsQueue = functor (Obj : SingleType) -> struct
  type obj = Obj.t
...
module ObjReqHashtbl (Id : SingleType) (Obj : SingleType) =struct
  type id = Id.t
  type obj = Obj.t
