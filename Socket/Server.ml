...
module type Server = functor (Cfg : ServerConfig) -> sig
...
module SimpleServer : Server = functor (Cfg : ServerConfig) -> struct
  ...
  (* To prevent multiple workers from trying to access the queue
     at the same time, we introduce a mutex and a function get_queue
     which tries to lock it before acting on the queue *)
  let obj_mutex = Mutex.create ()

  (* Request/objects queue utility functions *)
  (* /!\ This function locks the hashtable mutex /!\ *)
  let get_queue id =
